#!/usr/bin/env python3
"""One-off: create `wa_sent_log` tab in Customer Orders DB (GID 700800).

The GLOBAL send log. Every customer-facing n8n workflow writes here after sending WA.
Marketing workflows also READ here and respect a 7-day cooldown per phone to prevent
cross-workflow spam (e.g. a customer shouldn't get Reorder + Post-Trial in the same week).

Columns:
- phone        — normalized +65XXXXXXXX
- workflow     — workflow name (e.g. "reorder_reminder", "post_trial_nurture")
- template     — sub-template within workflow (e.g. "reminder_1", "D14", "winback_60d")
- sent_at      — ISO timestamp
- order_id     — related order if applicable
- notes        — freeform (e.g. "step=2", "cadence=14d")
"""
...
TAB_GID = 700800
TAB_NAME = "wa_sent_log"
HEADERS = ["phone", "workflow", "template", "sent_at", "order_id", "notes"]
...
def setup_tab():
    ...
    nodes = [
        {"parameters": {"httpMethod": "POST", "path": "tmp-add-wa-sent-log-tab",
    ...
    s, b = http("POST", "/workflows", {"name": "TEMP Setup WA Sent Log Tab",
    ...
    try:
        urllib.request.urlopen(urllib.request.Request(
            "https://thebonpet.app.n8n.cloud/webhook/tmp-add-wa-sent-log-tab",
            data=b'{}', method="POST",
